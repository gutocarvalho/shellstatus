...
MY_STATUS_CONFIG_DIR="$HOME/storage/status/config"

MY_STATUS_STATE_DIR="$HOME/storage/status/state"

MY_STATUS_PUBLIC_DIR="$HOME/storage/status/public"

MY_STATUS_TMP_DIR="$HOME/storage/status/tmp"
...
MY_STATUS_HTML="$MY_STATUS_PUBLIC_DIR/status_index.html"

# Where should the JSON status page be stored? Set to "" to disable JSON output
MY_STATUS_JSON="$MY_STATUS_PUBLIC_DIR/status.json"
...
MY_HOSTNAME_STATUS_HISTORY_TEMP_SORT="$MY_STATUS_TMP_DIR/status_hostname_history_sort.txt"
...
MY_STATUS_LOCKFILE="$MY_STATUS_TMP_DIR/STATUS_SH_IS_RUNNING.lock"
